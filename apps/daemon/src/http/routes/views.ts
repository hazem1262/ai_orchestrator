...
import { redactSavedView } from '../redact-out.ts';
import type { OrcApp } from '../types.ts';

export function registerViewRoutes(app: OrcApp, ctx: DaemonContext): void {
  app.get('/api/views', (c) => c.json(ctx.userMeta.views().map(redactSavedView)));
  app.post('/api/views', async (c) =>
    c.json(redactSavedView(ctx.userMeta.saveView(await readJson(c, SaveViewRequestSchema)))),
  );
